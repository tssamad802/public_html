...
{
    $perPage = new PerPage();
    $perPage->perpage = 12; // Preserve existing UI: 12 items per page
    $page = 1;
    ...
    $perpageresult = $perPage->getAllPageLinks($rowcount,'/ajax/ajax_home.php?actions=couponHomelisting&page=','searchfrm','resultDiv');
    ...
    { 
        ...
        echo "</div>";
        // -------- Output pagination ----------
        echo '<div class="col-sm-12 text-center">' . $perpageresult . '</div>';
    }
    else
    {
...
if($actions=="storeHomelisting")
{
    ...
    // -------- Count total rows ----------
    $countQuery = "SELECT COUNT(*) FROM tblstore s WHERE s.ShowHome = 1 AND s.active != 2";
    $db->query($countQuery);
    $db->next_record();
    $rowcount = $db->f(0);

    // -------- Main query with LIMIT ----------
    $queryproduct = "SELECT s.*, s.trackingUrl AS trackingLink, s.Active active, s.URLKeyword, s.TableID AS id, n.Title as NetName 
                     FROM tblstore s 
                     INNER JOIN `tblcountry` c ON (s.`CountryID` = c.`TableID`) 
                     INNER JOIN tblnetwork n ON (n.`TableID` = s.`NetworkID`) 
                     WHERE s.ShowHome = 1 AND s.active != 2 
                     ORDER BY s.name ASC 
                     LIMIT $start, $perPage->perpage";
    $db->query($queryproduct);

    $perpageresult = $perPage->getAllPageLinks($rowcount,'/ajax/ajax_home.php?actions=storeHomelisting&page=','searchfrm','resultDiv');
...
{
    $perPage = new PerPage();
    $perPage->perpage = 8; // Preserve existing UI: 8 items per page
    $page = 1;
    if(!empty($_REQUEST["page"])) {
        $page = $_REQUEST["page"];
    }
    ...
    // -------- Count total rows ----------
    $countQuery = "SELECT COUNT(*) FROM tblproduct p WHERE p.ShowHome = 1";
    $db->query($countQuery);
    $db->next_record();
    $rowcount = $db->f(0);

    // -------- Main query with LIMIT ----------
    $queryproduct = "SELECT p.*, country.Currency Currency 
                     FROM tblproduct p 
    ...
    $perpageresult = $perPage->getAllPageLinks($rowcount,'/ajax/ajax_home.php?actions=productHomelisting&page=','searchfrm','resultDiv');
