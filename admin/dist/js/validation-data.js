...
(function () {
  ...
  window.addEventListener(
    ...
    function () {
      ...
      if ($(".cropimages").length > 0) cropimages();
      ...
      $(".addplussign").click(function () {
        ...
        var anchorlink =
          ...
          masterpluscounter +
          ')" class="minussign"><i class="ion ion-ios-close-circle-outline"></i></a>';
        var htmltext =
          '<div class="otheplusbox removminus' +
          masterpluscounter +
          '">' +
          "<label >" +
          window.TxtEnglish +
...
function formvalidtecalss(newwformdata) {
  var forms =
    newwformdata == null
      ...
      : document.getElementsByClassName(newwformdata);
...
function SimpleAjax(pageUrl, FormId, divId) {
  ...
  jQuery.ajax({
    ...
    success: function (msg) {
      ...
      if ($(".sort-table").length > 0) SortItemData();

      if ($(".deleterecord").length > 0) confirmationDialog();
