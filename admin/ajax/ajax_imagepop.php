...
$sql="select * from  $TableName where TypeID = 1 order by TableID DESC";
$db->query($sql);
$RecordCount=0;
if($db->num_rows() > 0)
{
?>
	<table id="datable_1" class="table table-hover w-100 display pb-30">
         <thead>
              <tr>
                ...
                <th width="8%"  align="center"><?=IMAGES?></th> 
	...
	while($db->next_Record())
	{
		...
		<tr>
			...
			<td align="center"><span onclick="showallimagegallery('<?=$db->f('TableID')?>','6');"><i class="icon-camera"></i><span></td> 
            <td align="center" class=""><span class="badge <?=$StatusClass?>"><?=$Status?></span></td> 
            <td align="center">
           	 <a href="<?="index.php?".EncodeUrl("action=".$_REQUEST['action']."&SubLinkID=".$_REQUEST['SubLinkID']."&PageType=ManageRecord&RecordID=".$db->f('TableID')."&TypeID=6&ParentID=".$db->f('TableID')."&Trigger=edit")?>" class="iconhoverbox" title="<?=TXT_EDIT_RECORD?>"> <i class="icon-pencil"></i> </a>
