...
$TableName = "tblsystemusers_activelogin"; 
$FormAction = "AllFormDetails"; 
$refresh_div = 'resultDiv';

$sql="select L.TableID,LG.UserIP,LG.DateTime,U.UserName from  $TableName L 
	INNER JOIN tblsystemusers_login_log LG on LG.TableID=L.LogID 
	INNER JOIN tblsystemusers U on U.TableID=L.UserID
	where LG.UserIP != '".$_SERVER['REMOTE_ADDR']."' order by TableID DESC";
	/* $sql="select L.TableID,LG.UserIP,LG.DateTime,U.UserName from  $TableName L 
	INNER JOIN tblsystemusers_login_log LG on LG.TableID=L.LogID 
	INNER JOIN tblsystemusers U on U.TableID=L.UserID order by TableID DESC"; */
$db->query($sql);
$RecordCount=0;
if($db->num_rows() > 0)
{
?>
	<table id="datable_1" class="table table-hover w-100 display pb-30">
         <thead>
              <tr>
            	...
            	<th align="left"><?=TXT_LOGIN_DATETIME?></th>
				<th align="left"><?=TXT_LOGIN_TIME?></th>
	...
	while($db->next_Record())
	{
		...
		<tr>
			...
			<td align="left"><?=$db->f('UserName')?></td> 
			<td align="left"><?=onlydateshortformat($db->f('DateTime'));?></td> 
			<td align="left"><?=date('h:i:s a', strtotime($db->f('DateTime')));?></td>
			<td align="center">
             <?php if($CheckDeletePermissioon==1) { ?>
             <a class="deleterecord iconhoverbox" href="#" data-action_title="<?php echo TXT_DELETE_CONFIRM;?>" data-action_msg="<?php echo TXT_SELECTED_USER_LOGOUT;?>" data-message="<?php echo TXT_LOGOUT_USER_ACTION;?>" data-submitbtn="<?php echo LOGOUT;?>" data-logoutuser="1" data-action="<?=encodeencriptstring('DeleteRecord')?>" data-table="<?=encodeencriptstring($TableName)?>" data-id="<?=encodeencriptstring($db->f('TableID'))?>"  title="<?=TXT_SIGNOUT_USER?>"> <i class="zmdi zmdi-power"></i> </a>
             <?php } ?>
            </td>			
