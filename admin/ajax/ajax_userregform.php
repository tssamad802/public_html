<?php
require_once 'ajax.php'; 
$CheckDeletePermissioon = CheckModulePermission($UserRecordGetting['TableID'],$_REQUEST['SubLinkID'],"DeletePermissions");
if(isset($_REQUEST['FireAction']) && $_REQUEST['FireAction'] == 'listing')
{
//start search
/*$Active = $FullName = $UserName = $Email = $DepartmentID = $RoleID = '';
if(isset($_REQUEST['Active']))
	$Active = $_REQUEST['Active'];
if(isset($_REQUEST['FullName']))
	$FullName = $_REQUEST['FullName'];
if(isset($_REQUEST['UserName']))
	$UserName = $_REQUEST['UserName'];
if(isset($_REQUEST['Email']))
	$Email = $_REQUEST['Email'];
if(isset($_REQUEST['DepartmentID']))
	$DepartmentID = $_REQUEST['DepartmentID']; 
if(isset($_REQUEST['RoleID']))
	$RoleID = $_REQUEST['RoleID'];*/
/*$whereCond =''; 

if($FullName!='')
{
	$whereCond .= ' and (A.FullName LIKE "%'.$FullName.'%" )';
}
if($Active != '')
{
	$whereCond .= ' and A.Active = "'.$Active.'"';
}

if($UserName != '')
{
	$whereCond .= ' and A.UserName = "'.$UserName.'"';
}

if($Email != '')
{
	$whereCond .= ' and A.Email = "'.$Email.'"';
}
 

if($RoleID != '')
{
	$whereCond .= ' and A.RoleID = "'.$RoleID.'"';
}*/
$TableName = "tbluserregistration"; 
$FormName = "RegisterUserForm"; 
$FormAction = "AllFormDetails"; 
$refresh_div = 'resultDiv';

$sql="select U.*,N.Nationality,N.NationalityAr,C.Name,C.NameAr from  $TableName U 
	  INNER JOIN tblcountries N ON N.TableID = U.NationalityID 
	  INNER JOIN tblcountries C ON C.TableID = U.Country_ResidenceID 
	  order by TableID DESC";

$db->query($sql);
$RecordCount=0;
if($db->num_rows() > 0)
{
?>
	<table id="datable_1" class="table table-hover w-100 display pb-30">
         <thead>
              <tr>
                <th width="4%" align="center" ><?=SNO?></th> 
                <th align="left"><?=TXT_NAME?></th> 
                <th align="left"><?=TXT_GENDER?></th> 
                <th align="left"><?=TXT_DOB?></th> 
                <th align="left"><?=TXT_EMAIL?></th> 
            	<th align="left"><?=TXT_MOBILE?></th> 
                <th align="left"><?=TXT_NATIONALITY?></th>
                <th align="left"><?=TXT_COUNTRY_RESIDENCE?></th>
				<th width="10%" align="center" style="text-align:center;"><?=TXT_ACTION?></th>				
              </tr>
            </thead> 
        <tbody>
	<?php
	while($db->next_Record())
	{
		$RecordCount++;
		$Status = ($db->f('Active')==1)?TXT_ACTIVE:TXT_IN_ACTIVE;
        $StatusClass =   ($db->f('Active')==1)?'badge-success':'badge-danger';
		?>
		<tr>
			<td class="line-height" align="center"><?=$RecordCount?></td>
			<td align="left"><?=$db->f('FullName')?></td> 
			<td align="left"><?=($db->f('Gender')==1)?TXT_MALE:TXT_FEMALE;?></td> 
			<td align="left"><?=onlydateshortformat($db->f('DOB'))?></td> 
			<td align="left"><?=$db->f('Email')?></td> 
			<td align="left"><?=$db->f('Mobile')?></td> 
			<td align="left"><?=$db->f('Nationality'.LANG_SEP_DB)?></td>
			<td align="left"><?=$db->f('Name'.LANG_SEP_DB)?></td>
			<td align="center">
           	 <span onclick="showAllformDet('<?=encodeencriptstring($TableName)?>','<?=encodeencriptstring($FormName)?>','<?=encodeencriptstring($FormAction)?>','<?=encodeencriptstring($db->f('TableID'))?>')"> <i class="icon-eye"></i> </span> 
            </td>			
           
              
		</tr>
	<?php
	}?>
    	</tbody>
    </table>
    <?php
}
	if($RecordCount==0)
	{
		echo '<div class="norecordfound">'.DSB_NO_RECORDS.'</div>';
	}
	
	if($pagination->tot_pages > 1)
	{
		?>
		<tr>
			<td colspan="11">
				<center>
					  <?php echo $page_links;?>
				</center>
			</td>
		</tr>
		<?php
	}
}
